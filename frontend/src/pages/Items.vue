<template>
    <div class="p-4">
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
            <ItemCard v-for="item in items" :key="item.id" :item="item" cardType="ITEM" @purchase="handlePurchase" />
        </section>

        <!-- Confirmation Modal -->
        <div v-if="showPurchaseModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
            <div class="bg-white p-6 rounded shadow-lg w-96">
                <h2 class="text-lg font-bold mb-4">Confirm Purchase</h2>
                <p class="mb-4">Are you sure you want to purchase <strong>{{ selectedItem?.name }}</strong> for ${{
